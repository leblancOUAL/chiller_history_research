...
def main(argv=None):
    ...
    if args.command in ("report", "plots"):
        ...
        level_info = json.loads((Path(args.out) / "levels.json").read_text())
        if args.command == "report":
            stats = pipeline.compute_stats(minute, level_info)
            (Path(args.out) / "report.json").write_text(json.dumps(stats, indent=2))
            print_report(stats)
        else:
            summary = pd.read_csv(Path(args.out) / "monthly_summary.csv")
            plot_dir = make_plots(minute, summary, level_info, args.out)
            print(f"Wrote plots to {plot_dir}")
        return

    if args.command == "all":
        minute, level_info = pipeline.build(args.data_dir, args.out, args.start_year)
