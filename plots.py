...
def make_plots(minute, summary, level_info, out_dir):
    ...
    # 4: chiller state occupancy (duty cycle)
    if level_info:
        lv = pd.DataFrame(level_info)
        diffs = lv["level_kw"].diff().dropna()
        width = float(diffs.median()) * 0.8 if len(diffs) else 10.0
        fig, ax = plt.subplots(figsize=(10, 5))
        ax.bar(lv["level_kw"], lv["fraction_of_time"], width=width)
        ...
        ax.set_xlabel("chiller power level (kW)")
        ax.set_ylabel("fraction of time")
        ax.set_title("chiller duty cycle: time spent at each current level")
        fig.tight_layout()
